 ...
 * `/educate` still works and emits a deprecation notice; do not build on it.
 */
import {
  type Action,
...
export const educateNewcomerAction: Action = {
  ...
  validate: async (runtime: IAgentRuntime, message: Memory, state?: State) => {
    ...
    const hasIntroTopic = INTRO_TOPICS.some((topic) => text.includes(topic));
...
QUESTION: [main question]`;
      ...
      try {
        ...
        logger.debug(
          `🔍 [educateNewcomer] Searching knowledge for: "${searchQuery}"`,
        );
        ...
        logger.debug(
          `📚 [educateNewcomer] Found ${results?.length || 0} knowledge results`,
        );
        ...
        logger.error(
          `❌ [educateNewcomer] Knowledge search failed:`,
      ...
      logger.error("Error in EDUCATE_NEWCOMER action:", error);
...
function extractQuestion(assessment: string): string {
  ...
  const questionLine = lines.find((line) =>
    line.toUpperCase().includes("QUESTION:"),
  );
