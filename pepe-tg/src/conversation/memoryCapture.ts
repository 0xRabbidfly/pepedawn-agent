...
export function buildCapturePrompt(session: DayTurn[], known: KnownMemory[]): string {
  ...
  return [
    ...
    '',
    'salience: 1 worth keeping, 2 characteristic of them, 3 what the room would remember them for.',
    ...
    'Anyone who showed something of themselves here is worth one memory - you are building up a sense of',
    'these people over months, so a line that is merely characteristic of them counts, not only a great one.',
    'It does not have to be clever or quotable. Return {"memories":[]} only when the conversation really was',
    'all greetings and logistics, or when everything in it falls under "never remember" above.',
    '',
    ...lines,
  ].join('\n');
}
