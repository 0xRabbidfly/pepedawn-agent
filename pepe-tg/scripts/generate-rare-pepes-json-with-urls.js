#!/usr/bin/env node
/**
 * Generate Rare Pepes JSON with S3 URLs (no downloads)
...
const OUTPUT_JSON = path.join(__dirname, '../src/data/rare-pepes-data.json');

// Fetch from API
async function fetchAPI() {
  return new Promise((resolve, reject) => {
    https.get('https://api.pepe.wtf/api/asset?collection=rare-pepes', (res) => {
      let data = '';
...
async function main() {
  console.log('🔍 Fetching Rare Pepes API data...\n');
  ...
  for (const asset of testSamples) {
    const url = `https://pepewtf.s3.amazonaws.com/collections/rare-pepes/full/${asset.serie}/${asset.image}`;
  ...
  for (const asset of apiData) {
    ...
    const imageUrl = `https://pepewtf.s3.amazonaws.com/collections/rare-pepes/full/${asset.serie}/${asset.image}`;
